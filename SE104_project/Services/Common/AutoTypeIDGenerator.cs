...
{
    public class AutoTypeIDGenerator : IDGenerator
    {
        public AutoTypeIDGenerator(AppSession session, MongoClient client) : base(session, client)
        {
        }
        ...
        public async override Task<string> Generate()
        {
            ...
            var filter = Builders<BsonDocument>.Filter.Eq("_id", "SavedType");
            ...
            else if (ls.Count >= 1)
            {
                ...
                {
                    tmp = "TY" + (index + 1).ToString();
                    s = tmp;
                    var collectioncheck = database.GetCollection<BsonDocument>("ProductTypeInfomation");
