...
{
    public class AutoProductsIDGenerator: IDGenerator
    {
        public AutoProductsIDGenerator(AppSession session, MongoClient client) : base(session, client)
        {
        }
        ...
        public override async Task<string> Generate()
        {
            ...
            var filter = Builders<BsonDocument>.Filter.Eq("_id", "SavedProduct");
            ...
            else if (ls.Count >= 1)
            {
                ...
                {
                    tmp = "SP" + (index + 1).ToString();
                    s = tmp;
                    var collectioncheck = database.GetCollection<BsonDocument>("ProductsInformation");
